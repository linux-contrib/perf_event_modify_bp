...
int main(){
        ...
        for(int i = 0 ; i < N; i++){
                int fd = -1; 
		if(i & 1 == 0 ) 
			fd = create_watchpoint((uintptr_t) &dummy[i], HW_BREAKPOINT_R | HW_BREAKPOINT_W, HW_BREAKPOINT_LEN_2);
		else 
			fd = create_watchpoint((uintptr_t) &dummy[i+1], HW_BREAKPOINT_W | HW_BREAKPOINT_W, HW_BREAKPOINT_LEN_1);
	        dummy[i+1] = 1; 
        ...
        for(int i = 0 ; i < N; i++){
                if (i & 1 == 0)
                	modify_watchpoint(fd, (uintptr_t) &dummy[i], HW_BREAKPOINT_W | HW_BREAKPOINT_R, HW_BREAKPOINT_LEN_2);
 		else
                	modify_watchpoint(fd, (uintptr_t) &dummy[i+1], HW_BREAKPOINT_W | HW_BREAKPOINT_R, HW_BREAKPOINT_LEN_1);
	        dummy[i+1] = 1; 
        }
        distroy_watchpoint(fd);
        clock_t t4  = clock();
